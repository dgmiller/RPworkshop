...
# Parameter Recovery ------------------------------------------------------
nresp <- 5  # Number of respondents.
nscns <- 10 # Number of choice scenarios for each respondent.
nalts <- 3  # Number of alternatives in each choice scenario.
nlvls <- 4  # Number of attribute levels for each alternative.
...
# Save data in a list.
Data <- list(R = nresp, T = nscns, A = nalts, L = nlvls, C = ncovs,
...
# Trace plots.
plot(out_test, plotfun = "trace", pars = "B")

# Interval plots.
plot(out_test, pars = "B")

# Numeric summary.
summary(out_test, pars=c("B"))$summary
